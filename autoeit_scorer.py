...
if __name__ == '__main__':
    ...
    SOURCE = '/mnt/user-data/uploads/Example_EIT_Transcription_and_Scoring_Sheet.xlsx'
    OUTPUT = '/mnt/user-data/outputs/AutoEIT_Sample_Transcriptions_Scored.xlsx'

    # ── Run pipeline ────────────────────────────────────────────────────
    df = run_scoring_pipeline(SOURCE)

    # ── Print sentence-level sample ─────────────────────────────────────
    print("\n=== SENTENCE-LEVEL SCORES (sample — scored rows only) ===")
    sample = df[df['has_human_score']].head(40)[
    ...
    write_output_workbook(df, SOURCE, OUTPUT)

    csv_path = '/mnt/user-data/outputs/AutoEIT_scores.csv'
    df[['participant_id','version','sentence_id','stimulus',
        'transcription','human_score','auto_score','rationale']].to_csv(csv_path, index=False)
...
_FUNCTION_WORDS = {
    ...
    'que', 'se', 'le', 'lo', 'me', 'te', 'nos', 'les',
    'y', 'o', 'pero', 'sino', 'aunque', 'como', 'si', 'porque',
    'ya', 'no', 'ni', 'muy', 'mas', 'mas', 'su', 'sus',
    'mi', 'mis', 'tu', 'tus', 'al', 'del',
    'este', 'esta', 'estos', 'estas', 'ese', 'esa', 'esos', 'esas',
    'hay', 'es', 'son', 'esta', 'estan', 'era', 'eran', 'fue', 'fueron',
    'ha', 'han', 'he', 'hemos', 'haber',
    'ser', 'estar', 'tener', 'hacer'
}
...
def is_wrong_language(raw: str) -> bool:
    """
    Heuristic: if the rater noted '[en inglés]' or similar, score = 0.
    We detect this by checking for the phrase in the raw transcription.
    """
    if not isinstance(raw, str):
        return False
    lower = raw.lower()
...
_SER_FORMS   = {'es', 'son', 'era', 'eran', 'fue', 'fueron', 'sea', 'sean', 'ser', 'soy', 'somos'}
_ESTAR_FORMS = {'esta', 'están', 'estaba', 'estaban', 'estuvo', 'estuvieron', 'este', 'esten',
                'estar', 'estoy', 'estamos', 'estan'}

def only_ser_estar_swap(target_tokens: list[str], learner_tokens: list[str]) -> bool:
    """
    Returns True if the ONLY difference is a ser↔estar substitution.
    These are treated as score-3 deviations (minor, meaning preserved).
    """
    if len(target_tokens) != len(learner_tokens):
        return False
    diffs = [(t, l) for t, l in zip(target_tokens, learner_tokens) if t != l]
...
def score_utterance(target_raw: str, learner_raw: str) -> tuple[int, str]:
    ...
    # ── Normalise ────────────────────────────────────────────────────────
    target_tokens  = tokenise(target_raw)
    ...
    # Normalised edit distance (proportion of target that needed changing)
    norm_edit = edit_dist / n_target

    # ── Score 4 ──────────────────────────────────────────────────────────
    # Perfect or near-perfect: edit distance ≤ 1 word, content overlap ≥ 90%
    # (allows for missing punctuation, minor accent/capitalisation difference
    #  already removed by clean(), false starts already stripped)
    if edit_dist == 0:
        return 4, "Exact reproduction after normalisation"

    if edit_dist == 1 and overlap >= 0.90:
        # One-word difference — check if it's just an extra/missing function word
        # or a morphologically close variant
        diffs = []
        # Find differing positions via LCS alignment proxy
        t_set = set(target_tokens)
        l_set = set(learner_tokens)
        diff_words = (t_set - l_set) | (l_set - t_set)
        in_fw = all(w in _FUNCTION_WORDS for w in diff_words)
        return 4, (
            f"1-word deviation (function word only)" if in_fw
            else f"1-word deviation, content fully preserved (edit=1, overlap={overlap:.2f})"
        )

    # ── Score 3 ──────────────────────────────────────────────────────────
    # Minor deviation: meaning intact, small morphological/grammatical change
    #   • ser↔estar swap (same length)
    #   • edit_dist ≤ 2 AND content overlap ≥ 0.85
    #   • single content-word substitution that is morphologically close
    #   • 1 extra/missing word with high overlap

    if only_ser_estar_swap(target_tokens, learner_tokens):
        return 3, "Ser↔estar substitution; core meaning preserved"

    if edit_dist <= 2 and overlap >= 0.85:
        return 3, (
            f"Minor deviation (edit={edit_dist}, content_overlap={overlap:.2f}); "
            f"meaning essentially preserved"
        )

    # Check for single-content-word substitution that is morphologically close
    if len(target_tokens) == len(learner_tokens):
        mismatches = [(t, l) for t, l in zip(target_tokens, learner_tokens) if t != l]
        if len(mismatches) == 1:
            t_w, l_w = mismatches[0]
            if t_w not in _FUNCTION_WORDS and morph_close(t_w, l_w):
                return 3, (
                    f"Single morphological/phonological deviation: "
                    f"'{t_w}'→'{l_w}'; meaning preserved"
                )
            if t_w in _FUNCTION_WORDS or l_w in _FUNCTION_WORDS:
                return 3, (
                    f"Single function-word deviation: '{t_w}'→'{l_w}'; meaning preserved"
                )

        if len(mismatches) == 2 and overlap >= 0.75:
            return 3, (
                f"2 minor deviations, high content overlap ({overlap:.2f}); "
                f"meaning preserved"
            )

    # Edit distance 3 but very high overlap (e.g., false start added words)
    if edit_dist == 3 and overlap >= 0.88:
        return 3, (
            f"Small structural deviation (edit={edit_dist}), high content overlap "
            f"({overlap:.2f}); meaning preserved"
        )

    # ── Score 2 ──────────────────────────────────────────────────────────
    # Partial meaning: recognisable but notable deviation
    #   content overlap 0.45 – 0.85 OR edit_dist moderate
    if overlap >= 0.45 and n_learner >= max(1, n_target * 0.35):
        reason = (
            f"Partial meaning (content_overlap={overlap:.2f}, edit={edit_dist}, "
            f"n_target={n_target}, n_learner={n_learner})"
        )
        # Distinguish 2 vs 3 via overlap threshold
        if overlap >= 0.70:
            # High overlap but edit distance was > 3 — likely structural deviation
            return 3, reason.replace("Partial", "Near-accurate")
        return 2, reason

    # Short response with some overlap
    if overlap >= 0.30 and n_learner >= 2:
        return 2, (
            f"Partial response (content_overlap={overlap:.2f}, "
            f"n_learner={n_learner} tokens)"
        )

    # ── Score 1 ──────────────────────────────────────────────────────────
    # Minimal meaning — isolated recognisable fragments
    if overlap >= 0.15 or (n_learner >= 1 and overlap > 0):
        return 1, (
            f"Minimal meaning (content_overlap={overlap:.2f}, "
            f"n_learner={n_learner})"
        )

    # ── Score 0 ──────────────────────────────────────────────────────────
    return 0, (
        f"No meaning preserved (content_overlap={overlap:.2f}, "
        f"edit={edit_dist})"
    )
...
# ─────────────────────────────────────────────
#  EXCEL OUTPUT  — writes scores into a copy
#  of the original workbook, in-place per sheet
# ─────────────────────────────────────────────

# Colour palette matching the score semantics
SCORE_FILLS = {
    4: PatternFill(fill_type='solid', fgColor='C6EFCE'),   # green
    3: PatternFill(fill_type='solid', fgColor='FFEB9C'),   # yellow
    2: PatternFill(fill_type='solid', fgColor='FFCC99'),   # orange
...
def write_output_workbook(df: pd.DataFrame, source_path: str, out_path: str) -> None:
    """
    Writes the auto scores and rationale back into a copy of the original
    workbook format:
      • Col D (Score Rater 1) populated with auto_score  [if originally empty]
        or left if already scored (so we never overwrite existing human data)
      • Col E = AutoEIT_Score
      • Col F = Rationale
    Also adds a summary sheet.
    """
    from shutil import copyfile
    copyfile(source_path, out_path)
    ...
    for sheet_name in wb.sheetnames:
        ...
        # Build lookup: sentence_id → (auto_score, rationale)
        sub = df[(df['participant_id'] == pid) & (df['version'] == ver)]
        lookup = {int(r['sentence_id']): (r['auto_score'], r['rationale'])
                  for _, r in sub.iterrows()}

        # Ensure header row has the new columns
        ws.cell(row=1, column=5).value = 'AutoEIT_Score'
        ws.cell(row=1, column=6).value = 'Rationale'
        for col in [5, 6]:
            cell = ws.cell(row=1, column=col)
        ...
        for row_idx in range(2, ws.max_row + 1):
            ...
            rat_cell = ws.cell(row=row_idx, column=6)
            rat_cell.value = rationale
            rat_cell.alignment = Alignment(wrap_text=True, horizontal='left')
    ...
    headers = [
        'Participant', 'Version', 'Sentence', 'Stimulus',
        'Transcription', 'Human Score', 'AutoEIT Score', 'Score Diff', 'Rationale'
    ]
    for c, h in enumerate(headers, 1):
        cell = ws_sum.cell(row=1, column=c, value=h)
    ...
    for _, r in df.iterrows():
        hs   = r['human_score']
        auts = r['auto_score']
        diff = (int(auts) - int(hs)) if pd.notna(hs) else ''

        row_data = [
            r['participant_id'], r['version'], r['sentence_id'],
            r['stimulus'], r['transcription'],
            int(hs) if pd.notna(hs) else '',
            auts, diff, r['rationale']
        ]
        for c, val in enumerate(row_data, 1):
            cell = ws_sum.cell(row=row_idx, column=c, value=val)
            if c == 7:   # AutoEIT score — colour-coded
                cell.fill = SCORE_FILLS.get(int(auts), SCORE_FILLS[0])
                cell.alignment = Alignment(horizontal='center')
            if c == 8 and isinstance(diff, int) and diff != 0:
                cell.font = Font(color='FF0000', bold=True)
            cell.alignment = Alignment(wrap_text=True)
...
if __name__ == '__main__':
    ...
    sample = df[df['has_human_score']].head(30)[
        ['participant_id', 'version', 'sentence_id', 'stimulus',
         'transcription', 'human_score', 'auto_score', 'score_diff', 'rationale']
    ]
